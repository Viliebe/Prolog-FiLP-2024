...
problem(slow_work_and_noise):-
    query('The computer is running very slowly and the fan is making a lot of noise?').
...
problem(not_run):-
    query('The computer does not start?').
...
problem(on_pc):-
    query('Have you tried to turn on or restart your computer?').

fault(you_should_try_it):-
problem(on_pc).

problem(burning):-
    query('Is there a burning smell?').
